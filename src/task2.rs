...
pub fn solve(input: &str) -> i64 {
    let time: i64 = input.lines().next().unwrap().split(':').last().unwrap().split_whitespace().collect::<String>().parse().unwrap();

    let distance: i64 = input.lines().last().unwrap().split(':').last().unwrap().split_whitespace().collect::<String>().parse().unwrap();


    let (min, max) = extreme_solutions(&time, &distance);

    return count_solutions((min, max));
